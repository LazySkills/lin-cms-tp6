...
class Jwt
{
    ...
    public static function getHeaderAuthorization(){
        $authorization = request()->header(config('lincms.jwt.param'));
        ...
        if ($type !== config('lincms.jwt.type')) {
            throw new AuthenticationException('接口认证方式需为'.config('lincms.jwt.param'));
        }
    ...
    public static function check():void
    {
        ...
        try {
            static::$data = (array)\Firebase\JWT\JWT::decode($token, config('lincms.jwt.key'), ['HS256']);
    ...
    private static function create(bool $expire = true){
        $payload = config('lincms.jwt.payload');
        if (empty($payload)){
            throw new AuthenticationException('请检查lincms配置文件中jwt选项是否正确');
        }
        $payload['iat'] = time();
        $payload['uniqueId'] = static::$uniqueId;
        $payload['signature'] = static::$signature;
        if ($expire === true) {
            $payload['exp'] = $payload['iat'] + config('lincms.jwt.time');
        }else{
            unset($payload['exp']);
        }
